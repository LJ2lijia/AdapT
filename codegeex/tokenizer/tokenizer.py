...
class CodeGeeXTokenizer(object):
    ...
    def decode_code(self, input_ids):
        if self.mode == 'codegeex-13b':
            text = self.tokenizer.decode(input_ids, skip_special_tokens=False, verbose=False, spaces_between_special_tokens=False)
            # spaces_between_special_tokens=False should be used and can only be used on non-fast tokenizer
